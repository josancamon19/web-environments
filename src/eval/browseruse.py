...
ACTION_MAPPINGS = {
    "go_to_url": ActionMapping(
        agent_action="go_to_url", trajectory_action="go_to", key_mapping={"url": "url"}
    ),
...
def map_agent_action_to_trajectory(
    ...
    trajectory_action = {"type": mapping.trajectory_action, "params": {}}
        ...
        if agent_action.get("interacted_element"):
            ...
            if hasattr(element, "attributes"):
                ...
                if attrs.get("id"):
                    ...
                    trajectory_action["params"]["selector"] = (
                        f".{attrs['class'].replace(' ', '.')}"
                    )
                else:
...
def compare_actions(
    agent_action: Dict[str, Any], golden_action: Dict[str, Any]
) -> float:
...
def evaluate_task_execution(
    agent_actions: List[Dict[str, Any]], golden_trajectory: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for golden_idx, golden_action in enumerate(golden_trajectory):
        ...
        for agent_idx, agent_action in enumerate(converted_actions):
            ...
            position_diff = abs(
                agent_idx / max(len(converted_actions), 1)
    ...
    extra_agent_steps = len(
        [i for i in range(len(converted_actions)) if i not in matched_agent_indices]
    )
    ...
    if agent_actions:
        ...
        # If no explicit signal but all golden steps were matched with high confidence
            ...
            len(matched_golden) == len(golden_trajectory)
            and partial_credit >= len(golden_trajectory) * 0.8
        ):
    ...
    efficiency = (
        len(matched_golden) / total_agent_steps if total_agent_steps > 0 else 0.0
    )
...
async def run_task_with_agent(
    task_description: str, model: str = "gpt-5-2025-08-07"
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "actions": model_actions, "error": None}
...
async def process_all_tasks(model: str = "gpt-5-2025-08-07"):
    ...
    for i, task in enumerate(tasks):
        ...
        if agent_result["success"]:
            ...
            evaluation = evaluate_task_execution(
                agent_result["actions"], task["tool_calls"]
            )
        ...
        logger.info(
            f"Task {task['task_id']} - Accuracy: {result['accuracy']:.2%}, "
    ...
    avg_efficiency = (
        sum(r.get("efficiency", 0) for r in results if r["success"]) / successful_tasks
...
async def main():
    # await process_all_tasks("gpt-5-2025-08-07")
    await process_all_tasks("o3-2025-04-16")
