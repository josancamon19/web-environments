#!/usr/bin/env python3
"""
Upload script to upload the results directory to Google Cloud Storage.
Uploads results/ to gs://mind2web-subset/results/ in web-environments project.
"""
...
def get_files_to_upload(results_dir: Path) -> list[Path]:
    """Get list of all files to upload from results directory."""
    exclude_patterns = [
        "__pycache__",
        "*.pyc",
        ".DS_Store",
...
def main() -> None:
    ...
    console.print(f"[bold cyan]Uploading {RESULTS_DIR} to GCS[/bold cyan]\n")
    ...
    files_to_upload = get_files_to_upload(RESULTS_DIR)
    ...
    with Progress(
        ...
        for file_path in files_to_upload:
            try:
                # Calculate relative path from results_dir parent
                rel_path = file_path.relative_to(RESULTS_DIR.parent)
